...
namespace logging {
  ...
  enum class log_level : uint8_t { TRACE = 0, DEBUG = 1, INFO = 2, WARN = 3, ERROR = 4, NONE = 5 };
  ...
  //all, something in between, none or default to info
    ...
    log_level LOG_LEVEL_CUTOFF = log_level::TRACE;
  #elif defined(LOGGING_LEVEL_DEBUG)
    clog_level LOG_LEVEL_CUTOFF = log_level::DEBUG;
  #elif defined(LOGGING_LEVEL_WARN)
    log_level LOG_LEVEL_CUTOFF = log_level::WARN;
  #elif defined(LOGGING_LEVEL_ERROR)
    log_level LOG_LEVEL_CUTOFF = log_level::ERROR;
  #elif defined(LOGGING_LEVEL_NONE)
    log_level LOG_LEVEL_CUTOFF = log_level::ERROR + 1;
  #else
    log_level LOG_LEVEL_CUTOFF = log_level::INFO;
  #endif
  ...
  inline void setLogLevel(std::string logLevel) {
    if (logLevel == "TRACE") LOG_LEVEL_CUTOFF = log_level::TRACE;
    if (logLevel == "DEBUG") LOG_LEVEL_CUTOFF = log_level::DEBUG;
    if (logLevel == "INFO") LOG_LEVEL_CUTOFF = log_level::INFO;
    if (logLevel == "WARN") LOG_LEVEL_CUTOFF = log_level::WARN;
    if (logLevel == "ERROR") LOG_LEVEL_CUTOFF = log_level::ERROR;
